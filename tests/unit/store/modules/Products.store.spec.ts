...
describe('products store', () => {
  ...
  describe('actions', () => {
    ...
    it('filterByCategory filters exploreProducts and flashSaleProducts by matching slug', async () => {
      const store = createStore();
      store.commit('products/SET_PRODUCT_LIST', [mockProduct]);
      await store.dispatch('products/filterByCategory', 'beauty');
      expect(store.getters['products/exploreProducts']).toContainEqual(mockProduct);
      expect(store.getters['products/flashSaleProducts']).toContainEqual(mockProduct);
    });

    it('filterByCategory falls back to full productList when no products match the slug', async () => {
      const store = createStore();
      store.commit('products/SET_PRODUCT_LIST', [mockProduct]);
      await store.dispatch('products/filterByCategory', 'nonexistent-category');
      expect(store.getters['products/exploreProducts']).toContainEqual(mockProduct);
      expect(store.getters['products/flashSaleProducts']).toContainEqual(mockProduct);
    });
  });
